<%@page import="com.fei.bean.Goods"%>
<%@page import="com.fei.service.after.impl.GoodsTypeServiceImpl"%>
<%@page import="com.fei.service.after.GoodsTypeService"%>
<%@page import="com.fei.service.after.impl.GoodsServiceImpl"%>
<%@page import="com.fei.service.after.GoodsService"%>
<%@page import="com.fei.bean.GoodsType"%>
...
<%
	...
	List<GoodsType> typeList = gt.selectAll();
		...
		<div class="container">
			...
			<div class="collapse navbar-collapse"
				...
				<ul class="nav navbar-nav">
					<li><a href="<%=path%>/Goods/query">商品管理 <span
							class="sr-only">(current)</span></a></li>
					<li><a href="<%=path%>/GoodsType/query">商品类型管理</a></li>
					<li><a href="<%=path%>/User/query">用户管理</a></li>
					<li class="active"><a href="<%=path%>/Order/query">订单管理</a></li>
	...
	<div class="container" style="margin-top: 20px;">
		<form id="deleteForm"
			...
			<table style="text-align: center; margin-right: 10px; width: 99%"
				...
				<tbody>
					<c:forEach items="${orders}" var="order" varStatus="obj">

						<tr>
							<td><input type="checkbox" name="ordersid" value="${order.id}" /></td>
							<td>${order.id}</td>
...
<%--							</a></td>--%>
							...
							<td><a class="glyphicon glyphicon-trash" color="#5BC0DE"
								...
								href="<%=path%>/DeleteOrderServlet?ordersid=${order.id}"
								onclick="if(confirm('是否确定删除'+'${order.orderCode}'+'？')==false)return false"></a></td>
	...
	<div class="container">
		<nav aria-label="..." class="navbar-right" style="margin-right: 15px">
			<ul class="pagination">
				<li><a class=" btn-info"
					href="<%=path %>/Order/query?pageNo=1" aria-label="Previous">
			  ...
			  %>
				<li><a class=" btn-info"
					href="<%=path %>/Order/query?pageNo=${pageNo-1}"
				...
				<li><a class="btn"
					href="<%=path %>/Order/query?pageNo=${pageNo+1}">${pageNo}</a></li>
				<li><a class="btn-info"
					href="<%=path %>/Order/query?pageNo=${pageNo+1}&username=${user.username}"
				...
				<li><a class=" btn-info"
					href="<%=path %>/Order/query?pageNo=${pageNo}">${pageNo}</a></li>
				<li class="disabled"><a href="#" aria-label="Previous">»<span
						class="sr-only">(current) </span></a></li>
				<%
			 }
			 %>
				<li><a class=" btn-info"
					href="<%=path %>/Order/query?pageNo=${pageCount}"
